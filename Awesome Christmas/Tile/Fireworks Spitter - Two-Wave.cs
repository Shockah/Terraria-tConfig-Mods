...
public int GetData() {
	...
	while (items.Count < 10) items.Add(new Item());
	byte[] result = new byte[4];
	
	Color? c1, c2;
	c1 = ModWorld.Firework.GetColor(items[0],items[1],items[2],items[3],items[4]);
	c2 = ModWorld.Firework.GetColor(items[5],items[6],items[7],items[8],items[9]);
	...
	ModWorld.RgbToHsv(c2.Value.R,c2.Value.G,c2.Value.B,out h,out s,out v);
	result[2] = (byte)(h/360f*255f);
	result[3] = (byte)(s*255f);
...
public bool LaunchFirework(int t)
{
    ...
    Projectile.NewProjectile(Pos.X,Pos.Y,Velo.X,Velo.Y,"Holiday Firework - Two-Wave",30,2,Main.myPlayer);
	return true;
}
public void UseTile(Player p,int x,int y)
{
	...
	if (my > 0 && Main.tile[mx,my-1].active && Main.tile[mx,my-1].type == Main.tile[mx,my].type) {
		...
		if (Main.netMode == 0) {
			ModPlayer.GuiSpitter.Create(v,ModWorld.GetCustomItems(v),10);
