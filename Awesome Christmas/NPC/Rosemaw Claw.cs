public class LegCon
{
    ...
    public LegCon(NPC S,NPC E,float s1 = -1f,float s2 = -1f,float c1 = -1f,float c2 = -1f,Color C = default(Color))
    {
        ...
        //tex = Main.goreTexture[Config.goreID["Rosemaw CPath"]];
    ...
    public void Draw(SpriteBatch SP,int phase,Color special = default(Color))
    {
        ...
        //Texture2D TEX = Main.goreTexture[Config.goreID["Rosemaw CEnd"]];
...
public void AI()
{
    ...
    if(FirstFrame)
    {
        if(!Main.dedServ) MyLeg = new LegCon(M,N,0.8f,0.8f,100,100,Color.Tomato);
        FirstFrame = false;
        N.damage = 0;
