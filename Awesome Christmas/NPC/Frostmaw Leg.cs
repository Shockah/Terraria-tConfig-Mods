...
public void AI()
{
    ...
    if(FirstFrame)
    {
        if(N.ai[0] == 1.3f || N.ai[0] == -1.3f) if(!Main.dedServ) MyLeg = new LegCon(M,N,0.6f,0.7f,100,150,Color.Gray);
        if(N.ai[0] == 1f || N.ai[0] == -1f) if(!Main.dedServ) MyLeg = new LegCon(M,N,0.4f,0.6f,120,120,Color.Gray);
        FirstFrame = false;
