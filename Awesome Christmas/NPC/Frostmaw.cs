...
public void AI()
{
    ...
    if(FirstFrame)
    {
        ...
        if(Main.netMode != 1)
        {
            ...
            for(int i = 0; i < 2; i++)
            {
                int a = NPC.NewNPC((int)NC.X, (int)NC.Y, "Frostmaw Claw", N.whoAmI);
                NPC N2 = Main.npc[a];
                N2.ai[0] = (i==0?-1f:1f);
            ...
            for(int i = 0; i < 2; i++)
            {
                int a = NPC.NewNPC((int)NC.X, (int)NC.Y, "Frostmaw Leg", N.whoAmI);
                NPC N2 = Main.npc[a];
            ...
            for(int i = 0; i < 1; i++)
            {
                int a = NPC.NewNPC((int)NC.X, (int)NC.Y, "Frostmaw Ass", N.whoAmI);
                NPC N2 = Main.npc[a];
                N2.ai[0] = -1f;
                N2.ai[1] = (float)N.whoAmI;
                N2.target = N.target;
    ...
    if(!Main.dayTime && N.ai[0] > -310)
    {
        if(N.ai[0] > -1) 
        {
            string s = "Frostmaw Runs In Fear Of Something...!";
            if(Main.netMode == 0) Main.NewText(s,255,75,75);
            if (Main.netMode == 2)
            {
                NetMessage.SendData(25, -1, -1, s, 255, 75f, 75f, 255f, 0);
            }
            N.ai[0] = -1;
        }
        N.ai[0]--;
        if(Main.netMode!= 1 && N.ai[0] == -180)
        {
            NPC.NewNPC((int)NC.X,(int)NC.Y-3000,"Rosemaw");
        }
        if(N.ai[0] == - 300)
        {
            string s = "Rosemaw "+Lang.misc[16];
            if(Main.netMode == 0) Main.NewText(s,255,75,75);
            if (Main.netMode == 2)
            {
                NetMessage.SendData(25, -1, -1, s, 175, 75f, 255f, 255f, 0);
            }
        }
    }
    if(N.ai[0] < -1)
    {
        int dir = 1;
        if(NC.X/16 < Main.maxTilesX/2) dir = -1;
        N.velocity.X = 5f*dir;
    }
...
public void NPCLoot()
{
	...
	Item.NewItem((int)npc.position.X,(int)npc.position.Y,npc.width,npc.height,"Frozen Shard",1,false,0);
